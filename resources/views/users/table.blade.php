    ...
    <table class="table table-bordered table-striped table-hover table-sm">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center">
                    <!-- Edit Button -->
                    <button class="btn btn-warning btn-sm edit-user" data-id="{{ $user->id }}" data-name="{{ $user->name }}" data-username="{{ $user->username }}" data-email="{{ $user->email }}" data-role="{{ $user->role }}" data-status="{{ $user->is_active }}" data-bs-toggle="tooltip" title="Edit User">
                        <i class="bi bi-pencil-square"></i>
                    </button>

                    <!-- Delete Button -->
                    <button class="btn btn-danger btn-sm delete-user" data-id="{{ $user->id }}" data-name="{{ $user->name }}" data-bs-toggle="tooltip" title="Delete User">
                        <i class="bi bi-trash"></i>
                    </button>

                    <!-- Toggle Active/Deactivate Button -->
                    <button type="submit" class="btn btn-sm {{ $user->is_active ? 'btn-secondary' : 'btn-success' }} toggleStatus" data-id="{{ $user->id }}" data-name="{{ $user->name }}" data-status="{{ $user->is_active ? 'Deactivate' : 'Activate' }}" data-bs-toggle="tooltip" title="{{ $user->is_active ? 'Deactivate User' : 'Activate User' }}">
